	...
	if not GPIO.input(self.buttonPin):
		if (self.currentPct > percent):
			...
			while self.currentPct > percent:
				...
				self.pwm.ChangeDutyCycle(self.scalePercentageToDutyCycle(self.currentPct))
...
class PlaySide(object):
	...
	GPIO.add_event_detect(self.pirScorePin, GPIO.FALLING, callback=self.scoreChangeSensedEvent, bouncetime=2500)
...
fans = [ FanDevice(2, MOTORS[0].MAX_PERCENTAGE, "0", "fan"), FanDevice(9, MOTORS[1].MAX_PERCENTAGE, "1", "fan") ]
...
PLAY_SIDES = [ PlaySide(MOTORS[0], fans[0], GPIO_PIN_PIR_1,GPIO_SWITCH_1), PlaySide(MOTORS[1], fans[1], GPIO_PIN_PIR_2,GPIO_SWITCH_2) ]
